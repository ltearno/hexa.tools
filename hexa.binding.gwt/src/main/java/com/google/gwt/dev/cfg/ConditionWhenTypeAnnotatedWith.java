...
/**
 * A deferred binding condition to determine whether the type is
 * annotated with a particular annotation type.
 */
// TODO(stalcup): guard against attempts to replace classes that have special prototype handling
// like String and Array.
public class ConditionWhenTypeAnnotatedWith extends Condition {

  private static boolean warnedMissingValidationJar = false;

  private final String annotatedWithTypeName;

  public ConditionWhenTypeAnnotatedWith(String assignableToTypeName) {
    this.annotatedWithTypeName = assignableToTypeName;
  }

  @Override
  public boolean equals(Object object) {
    if (object instanceof ConditionWhenTypeAnnotatedWith) {
      ConditionWhenTypeAnnotatedWith that = (ConditionWhenTypeAnnotatedWith) object;
      return Objects.equal(this.annotatedWithTypeName, that.annotatedWithTypeName);
    }
    return false;
  }

  public String getAnnotatedWithTypeName() {
    return annotatedWithTypeName;
  }

  @Override
  public int hashCode() {
    return Objects.hashCode(annotatedWithTypeName);
  }
  ...
  public String toSource() {
    ...
    return String.format("@com.google.gwt.lang.Cast::canCastClass(*)(requestTypeClass, @%s::class)",
        annotatedWithTypeName);
  }

  @Override
  public String toString() {
    return "<when-type-annotated class='" + annotatedWithTypeName + "'/>";
  }
  ...
  protected boolean doEval(TreeLogger logger, DeferredBindingQuery query)
    ...
    JClassType checkType = typeOracle.findType(annotatedWithTypeName);
    if (checkType == null) {
      ...
      if (shouldSuppressWarning(logger, annotatedWithTypeName)) {
        // Suppress validation related errors
        level = TreeLogger.DEBUG;
      }
      logger.log(level, "Unknown type '" + annotatedWithTypeName
          + "' specified in deferred binding rule", null);
      return false;
    }

    if(!(checkType instanceof JAnnotationType)) {
      // We only accept annotation types
      //
      TreeLogger.Type level = TreeLogger.WARN;
      if (shouldSuppressWarning(logger, annotatedWithTypeName)) {
        // Suppress validation related errors
      ...
      logger.log(level, "Type '" + annotatedWithTypeName
          + "' is not an annotation, specified in deferred binding rule", null);
    }

    for(Annotation a : fromType.getAnnotations()) {
      if(a.annotationType().getName().equals(annotatedWithTypeName)) {
        return true;
      }
    }
    return false;
  ...
  protected String getEvalAfterMessage(String testType, boolean result) {
    if (result) {
      return "Yes, the requested type is annotated with";
    } else {
      return "No, the requested type is not annotated with";
    }
  }
