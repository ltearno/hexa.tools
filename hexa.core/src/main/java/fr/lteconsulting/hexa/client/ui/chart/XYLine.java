...
public class XYLine implements ChartRemovable
{
	...
	/*
	 * JUST FOR MEMORY : HOW TO PROCESS THE SPline Anchor points, but the
	 * algorithm is not so cool eventually
	 */
	// double x0 = xs[i-1];
	// double y0 = ys[i-1];
	//
	...
	// double tension = 15;
	// double lenght1X = Math.sqrt( ( x - x0 ) * ( x - x0 ) + ( y - y0 ) * ( y -
	// y0 ) ) / tension;// / 7;
	// double lenght2X = Math.sqrt( ( x2 - x ) * ( x2 - x ) + ( y2 - y ) * ( y2
	// - y ) ) / tension;// / 7;
}
