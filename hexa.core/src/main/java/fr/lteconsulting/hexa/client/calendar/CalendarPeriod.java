...
public class CalendarPeriod {
    ...
    private List<Period> _Intersect(List<Period> periods1, List<Period> periods2) {
        ...
        while (i < count1 && j < count2) {
            ...
            if (periods1.get(i).getFrom().compareTo(periods2.get(j).getTo()) > 0) { // period 1 begins after period 2 finishes => period2 is
                // eliminated !
                j++;
            } else if (periods2.get(j).getFrom().compareTo(periods1.get(i).getTo()) > 0) { // period 2 begins after end of period 1 => period 1 is eliminated
                // !
                i++;
            }
