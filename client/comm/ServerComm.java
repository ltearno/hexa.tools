...
public class ServerComm
{
	...
	private IUserLogInfo userLogInfo = null;
	private ServerCommMessageCb serverCommMessageCb = null;
	private String baseUrl = null;
	
	
	public void Init( String baseUrl, IUserLogInfo userLogInfo, ServerCommMessageCb serverCommMessageCb, Service service )
	{
	...
	private void sendTimerEvent()
	{
		...
		String currentService = requestsToSend.get(0).request.service;
		String currentInterfaceChecksum = requestsToSend.get(0).request.interfaceChecksum;
		
		NetworkPendingRequestInfo networkRqInfo = new NetworkPendingRequestInfo( rqCallback );
		
		// Session part of the url
		IUserLogInfo.ISessionUse session = userLogInfo.getSessionUse();
		if( session != null )
		{
			JSONArray logUserWithSessionParams = new JSONArray();
			logUserWithSessionParams.set( logUserWithSessionParams.size(), new JSONString( session.getSessionId() ) );
			logUserWithSessionParams.set( logUserWithSessionParams.size(), new JSONNumber( session.getIteration() ) );
			logUserWithSessionParams.set( logUserWithSessionParams.size(), new JSONString( session.getChallengeAnswer() ) );
			RequestDesc desc = new RequestDesc( currentService, currentInterfaceChecksum, "logUserWithSession", logUserWithSessionParams );
			
			RequestCallInfo info = new RequestCallInfo( desc, true );
			
			networkRqInfo.toSend( info );
		}
		
		int count = 0;
		while( requestsToSend.size()>0 && requestsToSend.get(0).request.service.equals( currentService ) && count<MAX_NB_CALLS_BY_REQUEST )
		{
	...
	XNetworkPendingRequestInfo rqCallback = new XNetworkPendingRequestInfo()
	{
		...
		public void error( ErrorCodes errorCode, Exception exception, NetworkPendingRequestInfo request )
		{
			...
			case ERROR_REQUEST_RESPONSE_EMPTY:
				//Window.alert( "Empty server response for request, status code: " + request.getResponse().getStatusCode() );
				break;
		...
		public void answer( RequestCallInfo info, int msgLevel, String msg, GenericJSO hangOut, ResponseJSO retValue, NetworkPendingRequestInfo request )
		{
			...
			// specific processes of answer
			if( info.fIsLogUserWithSession )
			{
				// special user log info request
				userLogInfo.setUserJSO( retValue.getJSO(0) );
			}
			else if( hangOut != null )
			{
