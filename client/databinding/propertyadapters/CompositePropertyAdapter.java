...
public class CompositePropertyAdapter implements PropertyAdapter
{
	...
	// create adapaters from the root context object to the end of the path, if possible...
	private void tryCreateAdapters()
	{
		...
		for( int p=0; p<path.length; p++ )
		{
			...
			if( adapters[p] == null )
			{
				...
				// context is the 'object' value (ie the value of the previous pathItem or the root context)
				// path item is path[p]
				...
				// we should subscribe to the value changes so that we can subscribe to 
