...
public class Validator<T extends Widget> extends Composite implements ClickHandler, KeyUpHandler
{
	...
	public void onKeyUp(KeyUpEvent event)
	{
		...
		/*else if( event.getCharCode() == KeyCodes.KEY_TAB ) {
			...
			m_callback.onValidatorMoveRequest( 0, -1 );
		}
		else if( event.getCharCode() == KeyCodes.KEY_DOWN ) {
			m_callback.onValidatorMoveRequest( 0, 1 );
		}
		else if( event.getCharCode() == KeyCodes.KEY_LEFT ) {
			m_callback.onValidatorMoveRequest( -1, 0 );
		}
		else if( event.getCharCode() == KeyCodes.KEY_RIGHT ) {
			m_callback.onValidatorMoveRequest( 1, 0 );
		}*/
		else {
			return;
