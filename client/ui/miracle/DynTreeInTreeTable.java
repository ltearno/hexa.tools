...
public class DynTreeInTreeTable<T,H> implements Prints<Iterable<T>>, DynArrayManager<T>
{
		...
		if( userComparator != null )
		{
			for( Item item : items )
			{
				...
				if( userComparator.compare( object, refMng.getObject( refAtRow ) ) <= 0 )
				{
					...
					{
						// keep the same place only if lesser than the nxt item or if there is no next
	...
	class PrinterFirstCol implements Printer
	{
		...
		private String getExpShrinkHTML()
		{
			...
			{
				if( item.getExpanded() )
					return "<img src='"+HexaFramework.images.treeMinus().getURL()+"'/>";
				else
					return "<img src='"+HexaFramework.images.treeMinus().getURL()+"'/>";
			}
			else
				return "<img src='"+HexaFramework.images.blank16().getURL()+"'/>";
		}
	}
	...
	DragDrop.Callback<Integer> onDragDrop = new DragDrop.Callback<Integer>() {
		...
		{
			//return "<div style='background-color:rgba(120,120,120,0.4);'>Moving column "+cookie+"</div>";
			...
			// Due to asynchronism, it may be possible that the printer did not yet print anything, well... Too late, we use the source element instead !!
	...
	private TreeTableHandler tableHandler = new TreeTableHandler() {
		...
		public void onTableCellClick( Item item, int column, ClickEvent event )
		{
			...
			if( column == 0 )
			{
				...
				// else, offer the possibility to edit the cell, or else send the click event to click mng
