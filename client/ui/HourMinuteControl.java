...
public class HourMinuteControl extends Composite {
	...
	private static HourMinuteControlUiBinder uiBinder = GWT
			.create(HourMinuteControlUiBinder.class);

	interface HourMinuteControlUiBinder extends
			UiBinder<Widget, HourMinuteControl> {
	}
	
	@UiField ListBox hour;
	@UiField ListBox minute;
	...
	{
		return new HexaTime( 
				Integer.parseInt( hour.getValue(hour.getSelectedIndex()) ), 
				Integer.parseInt( minute.getValue(minute.getSelectedIndex()) ), 0 );
	}
}
