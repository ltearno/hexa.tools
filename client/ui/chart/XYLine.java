...
public class XYLine implements ChartRemovable {
	Layer layer = null;
	...
	int lineStroke = 1; // 2
	float dotNormalSize = (float)0.5; // 4
	int dotBigSize = 4; // 7
	...
	public void update()
	{
		...
		// TODO : this is a hack to get the dot stroke color the same as the line's color
	...
	/*
	 * JUST FOR MEMORY : HOW TO PROCESS THE SPline Anchor points, but the algorithm is not so cool eventually
	 */
//	double x0 = xs[i-1];
//	double y0 = ys[i-1];
//	
...
//	double y2 = ys[i+1];
	...
	// at PTi, we calculate the derivative of PTi-1 to PTi+1
//	double slope = ( y2 - y0 ) / ( x2 - x0 );
//	double angle = Math.atan( slope );
//	double tension = 15;
//	double lenght1X = Math.sqrt( ( x - x0 ) * ( x - x0 ) + ( y - y0 ) * ( y - y0 ) ) / tension;// / 7;
//	double lenght2X = Math.sqrt( ( x2 - x ) * ( x2 - x ) + ( y2 - y ) * ( y2 - y ) ) / tension;// / 7;
}
