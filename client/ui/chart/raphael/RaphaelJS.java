...
public class RaphaelJS extends JavaScriptObject
{
	...
	public static class Element extends JavaScriptObject
	{
		 ...
		 * @attributes_to_animate: . clip-rect string . cx number . cy number . fill colour . fill-opacity number . font-size number . height number . opacity
		 *                         number . path pathString . r number . rotation number . rx number . ry number . scale string . stroke colour . stroke-opacity
		 *                         number . stroke-width number . translation string . width number . x number . y number
		 * @param duration
		...
		public final native Element animate( JavaScriptObject newAttrs, int ms, AnimationCallback callback ) /*-{
			return this
					.animate(
							...
							ms,
							function() {
								@com.hexa.client.ui.chart.raphael.AnimationCallback::fire(Lcom/hexa/client/ui/chart/raphael/AnimationCallback;)(callback);
							});
		}-*/;
		 ...
		 * @attribute_names: . clip-rect string comma or space separated values: x, y, width and height . cx number . cy number . fill colour or gradient .
		 *                   fill-opacity number . font string . font-family string . font-size number . font-weight string . height number . opacity number .
		 *                   path pathString . r number . rotation number . rx number . ry number . scale comma or space separated values: xtimes, ytimes, cx,
		 *                   cy. . src string (URL) . stroke colour . stroke-dasharray string [â€œâ€�, â€œ-â€�, â€œ.â€�, â€œ-.â€�, â€œ-..â€�, â€œ. â€�, â€œ-
		 *                   â€�, â€œ--â€�, â€œ- .â€�, â€œ--.â€�, â€œ--..â€�] . stroke-linecap string [â€œbuttâ€�, â€œsquareâ€�, â€œroundâ€�] . stroke-linejoin
		 *                   string [â€œbevelâ€�, â€œroundâ€�, â€œmiterâ€�] . stroke-miterlimit number . stroke-opacity number . stroke-width number .
		 *                   translation string comma or space separated values: x and y . width number . x number . y number
		 * @param value
	...
	/**
	 * Added by saurabh tripathi 21/7/2011 There is an inconvenient rendering bug is Safari (WebKit): sometimes the rendering should be forced. This method
	 * should help with dealing with this bug.
	 */
