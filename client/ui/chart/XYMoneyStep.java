...
public class XYMoneyStep implements ChartRemovable
{
	Layer layer = null;
	...
	private void updateLineAndFill()
	{
		...
		//fillPath.M( ref.getRealX(ref.getMinX()), ref.getRealY(0) );
		...
		for( int i = 0; i<nbPoints; i++ )
		{
			...
			if( i == 0 )
			{
				...
				fillPath.M( realX, ref.getRealY(0) );
			...
			else
			{
				...
				fillPath.L( realX, ref.getRealY(0) );
				fillPath.L( ref.getRealX(ref.getMinX()), ref.getRealY(0) );
			}
			
			if( i < nbPoints - 1 )
			{
				float nextX = xs.get( i + 1 );
				float realNextX = ref.getRealX( nextX );
				
				linePath.L( realNextX, realY );
				fillPath.L( realNextX, realY );
			}
		}
