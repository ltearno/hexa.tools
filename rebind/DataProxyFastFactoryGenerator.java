...
import com.hexa.client.comm.FactoredInterface;
...
public class DataProxyFastFactoryGenerator extends Generator
{
	...
	public static void generateDataImpl( String className, JType type, SourceWriter sw, TreeLogger logger )
	{
		...
		//sw.println( "{" );
		//sw.indent();
		//sw.println( "public static int _sMakeHere = 10;" );
		
		/*sw.println( "public static "+type.getSimpleSourceName() + "ImplStd as( JavaScriptObject obj )" );
		sw.println( "{" );
		sw.println( "GWT.log( \"Casting obj : \" + obj.toString() );" );
		sw.println( "return obj.cast();" );
		sw.println( "}" );*/
	...
	public static void generate( JType type, SourceWriter sw, TreeLogger logger )
	{
		...
		//sw.println( type.getSimpleSourceName() + "ImplStd impl = "+type.getSimpleSourceName() + "ImplStd.as( obj );" );
	...
	public String generate(TreeLogger logger, GeneratorContext ctx,
	    ...
	    String createdClassName = remoteService.getSimpleSourceName()+"Impl";//requestedClassName + "Impl";
	    String fullCreatedClassName = "com.example.client.data.record."+remoteService.getSimpleSourceName()+"Impl";//requestedClass + "Impl";
	    String packageName = "com.example.client.data.record";//remoteService.getPackage().getName();
	    ...
	    if( pw2 != null )
	    {
		    ...
		    cf2.addImplementedInterface( fiType.getParameterizedQualifiedSourceName() );
		    cf2.setSuperclass( "GenericJSO" );
		  //sw.println( "static class "+getDataProxyFastImplName(type)+" extends GenericJSO implements "+type.getParameterizedQualifiedSourceName() );
		    SourceWriter sw2 = cf2.createSourceWriter( ctx, pw2 );
		    generateDataImpl( fiType, sw2, logger );
		    sw2.commit( logger );
	    }
	    */
		...
		composerFactory.addImport( "com.hexa.client.tools.HexaTools" );
